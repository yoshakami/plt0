                        ...
                        case 0xE: // CMPR
                            {
                                 ...
                                 * I'll be directly storing blocks here because the rgb565 values can't be added like that lol */
                                    ...
                                    default: // RRRR RGGG GGGB BBBB
                                        {
                                            for (y = pixel_data_start_offset; y < bmp_filesize; y += 4)
                                            {
                                                ...
                                                if (j == 4)
                                                {
                                                    j = 0;
                                                    z++;
                                                    y += (bitmap_width << 2) - 20; // returns to the start of the next line (I hope)  - bitmap width << 2 because it's a 32-bit BGRA bmp file
                                                    if (z == 4)
                                                    {
                                                        alpha_bitfield = 0;
                                                        use_alpha = false;
                                                        ...
                                                        {
                                                            y -= (bitmap_width << 4) + 12; // on retire 4 lignes et on passe le 1er block héhé
                                                        ...
                                                        if (j < colour_number_x2) // if the colour palette is not full
                                                        {
                                                            ...
                                                            if (j < colour_number_x2) // if the colour palette is still not full
                                                            {
                                                                ...
                                                                for (int i = 0; i < 16 && c < 4; i++)
                                                                {
                                                                    ...
                                                                    {
                                                                        index[c] = (byte)(Colour_list[i][1] >> 8);  // adds the RRRR RGGG value
                                                                        index[j + 1] = (byte)(Colour_list[i][1]);  // adds the GGGB BBBB value
                                                                        j += 2;
                                                                    }
                                                                }
                                                            }
                                                    ...
                                                    for (int h = 0; h < 4; h++)
                                                    {
                                                        for (int w = 0; w < 4; w++)  // index_size = number of pixels
                                                        {
                                                            ...
                                                            for (byte i = 0; i < Colour_palette.Count; i++)  // process the colour palette to find the closest colour corresponding to the current pixel
                                                            {
                                                                ...
                                                                {
                                                                    diff = (short)(Math.Abs(((Colour_palette[i] >> 8) & 248) - ((Colour_rgb565[(h * 4) + w] >> 8) & 248)) + Math.Abs(((Colour_palette[i] >> 3) & 252) - ((Colour_rgb565[(h * 4) + w] >> 3) & 252)) + Math.Abs(((Colour_palette[c] << 3) & 248) - ((Colour_rgb565[(h * 4) + w] << 3) & 248)));
                                                                    if (diff < diff_min)
                                                                    {
                                                                        diff_min = diff;
                                                                        diff_min_index = (byte)(i >> 1);
                                                            ...
                                                            index[2 + h] += (byte)(diff_min_index << (w << 1));
                                                            c += 1;
                                                        }
                                                        /*
                                                        pixel = (ushort)(Math.Abs(block_width - bitmap_width) % block_width);
