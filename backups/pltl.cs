    ...
    class plt0_class
    {
                    ...
                    if (input_file2 != "")
                    {
                        using (System.IO.FileStream file = System.IO.File.Open(input_file2, System.IO.FileMode.Open, System.IO.FileAccess.Read))
                        {
                            ...
                            else
                            {
                                ...
                                int bmp_size = bmp_image[2] | bmp_palette[3] << 8 | bmp_palette[4] << 16 | bmp_palette[5] << 24;
                                    ...
                                    case 0: // AI8
                                        {
                                            switch (algorithm)
                                            {
                                                case 0: // cie_601
                                                    {
                                                        for (int i = pixel_start_offset, j = 0; i < bmp_size; i += 4, j += 2)  // process every pixel to fit the AAAA AAAA  CCCC CCCC  profile
                                                        {
                                                            colour_palette[j] = (bmp_palette[i + 3]);  // alpha value
                                                            if (bmp_palette[i + 3] != 0)
                                                            {
                                                                colour_palette[j + 1] = (byte)(bmp_palette[i] * 0.114 + bmp_palette[i + 1] * 0.587 + bmp_palette[i + 2] * 0.299);
                                                            }
                                                        }
                                                        break;
                                                    }
                                                case 1: // cie_709
                                                    {
                                                        for (int i = pixel_start_offset, j = 0; i < bmp_size; i += 4, j += 2)  // process every pixel to fit the AAAA AAAA  CCCC CCCC  profile
                                                        {
                                                            ...
                                                            if (bmp_palette[i + 3] != 0)
                                                            {
                                                                colour_palette[j + 1] = (byte)(bmp_palette[i] * 0.0721 + bmp_palette[i + 1] * 0.7154 + bmp_palette[i + 2] * 0.2125);
                                                            }
                                                        }
                                                        break;
                                                    }
                                                case 2:  // custom
                                                    {
                                                        for (int i = pixel_start_offset, j = 0; i < bmp_size; i += 4, j += 2)  // process every pixel to fit the AAAA AAAA  CCCC CCCC  profile
                                                        {
                                                            ...
                                                            if (bmp_palette[i + 3] != 0)
                                                            {
                                                                colour_palette[j + 1] = (byte)(bmp_palette[i] * custom_rgba[2] + bmp_palette[i + 1] * custom_rgba[1] + bmp_palette[i + 2] * custom_rgba[0]);
                                                            }
                                                        }
                                                        break;
                                                    }
                                    ...
                                    case 1:  // RGB565
                                        {
                                            ...
                                            switch (algorithm)
                                            {
                                                case 2:  // custom  RRRR RGGG GGGB BBBB
                                                    {
                                                        for (int i = pixel_start_offset, j = 0; i < bmp_size; i += 4, j += 2)
                                                        {
                                                            red = (byte)(bmp_palette[i + 2] * custom_rgba[0]);
                                                            green = (byte)(bmp_palette[i + 1] * custom_rgba[1]);
                                                            blue = (byte)(bmp_palette[i] * custom_rgba[2]);
                                                ...
                                                case 2:  // custom
                                                    {
                                                        if (alpha == 1)  // 0AAA RRRR GGGG BBBB
                                                        {
                                                            for (int i = pixel_start_offset, j = 0; i < bmp_size; i += 4, j += 2)
                                                            {
                                                                a = (byte)(bmp_palette[i + 3] * custom_rgba[3]);
                                                                red = (byte)(bmp_palette[i + 2] * custom_rgba[0]);
                                                                green = (byte)(bmp_palette[i + 1] * custom_rgba[1]);
                                                                blue = (byte)(bmp_palette[i] * custom_rgba[2]);
        ...
        public void write_BMP(List<List<byte[]>> index_list)  // index_list contains all mipmaps.
        {
            ...
            for (int i = 0; i < mipmaps_number + 1; i++)
            {
                ...
                data[34] = (byte)(size);
                data[35] = (byte)(size >> 8);
                data[36] = (byte)(size >> 16);
                data[37] = (byte)(size >> 24);
                data[38] = 0;
                data[39] = 0;
                data[40] = 0;
                ...
                {
                    for (int k = 0; k < bitmap_width; k++, index++)
                    {
