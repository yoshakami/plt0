...
namespace plt0
{
    ...
    class plt0_class
    {
        ...
        [MethodImpl(MethodImplOptions.NoOptimization)]
        public void write_TEX0(List<byte[]> index_list)  // index_list contains all mipmaps.
        {
            int size = 0x40;
            double temp;
            int[] param = new int[4];
            ...
            {
                cursor = 0;
                for (int j = 0; j < settings[i][1] - 1; j++)
                {
                    for (int k = 0; k < settings[i][0] - 1; k++)
                    {
                        for (int l = 0; l < block_height; l++)
                        {
                            for (int m = 0; m < block_width; m++)
                            {
                                tex_data[count] = index_list[i][cursor];
                                cursor++;
                                count++;

                            }
                            cursor += settings[i][2] - block_width;
                        }
                        cursor = j * block_height * settings[i][0] * block_width + k * block_width;
                    }
                    for (int l = 0; l < block_height; l++)
                    {
                        for (int m = 0; m < settings[i][2] % block_width; m++)
                        {
                            tex_data[count] = index_list[i][cursor];
                            cursor++;
                            count++;
                        }
                        if (settings[i][3] % block_width == 0)
                        {
                            for (int m = settings[i][2] % block_width; m < block_width; m++)
                            {
                                tex_data[count] = index_list[i][cursor];
                                cursor++;
                                count++;
                ...
                for (int k = 0; k < settings[i][0] - 1; k++)
                {
                    for (int l = 0; l < settings[i][3] % block_height; l++)
                    {
                        for (int m = 0; m < block_width; m++)
                        {
                            tex_data[count] = index_list[i][cursor];
                    ...
                    if (settings[i][3] % block_height == 0)
                    {
                        for (int l = 0; l < block_height; l++)
                        {
                            for (int m = 0; m < block_width; m++)
                    ...
                    {
                        for (int l = settings[i][3] % block_height; l < block_height; l++)
                        {
                            for (int m = 0; m < block_width; m++)
                            {
                                tex_data[count] = 0;
                    ...
                    cursor = (settings[i][1] - 1) * block_height * settings[i][0] * block_width + k * block_width;
                }
                for (int l = 0; l < settings[i][3] % block_height; l++)
                {
                    for (int m = 0; m < settings[i][2] % block_width; m++)
        ...
        {
            short pixel;
            List<short> Colours = new List<short>();
                ...
                case 32:
                    {
                        ...
                        {
                            diff_min = 500;
                            for (int i = 0; i < colour_number_x2; i += 2)
                            {
                                if (colour_palette[i] == (byte)(Colours[k] >> 8) && colour_palette[i + 1] == (byte)Colours[k])
                                {
                                ...
                                {
                                    diff = (short)(Math.Abs(colour_palette[i] - (byte)(Colours[k] >> 8)) + Math.Abs(colour_palette[i + 1] - (byte)Colours[k]));
                                    if (diff < diff_min)
                                    {
                                        diff_min = diff;
                                        diff_min_index = (byte)(i / 2);
        ...
        public byte[] create_PLT0_IA8(byte[] bmp_image, int bmp_filesize, int pixel_data_start_offset)
        {
            ...
            byte[] index = new byte[index_size]; // will contains a row of pixel index starting by the bottom one because bmp_image starts by the bottom one
            ...
            switch (bmp_image[0x1C])  // colour depth
            {
                case 32:  // 32-bit BGRA bmp image
                    {
                        switch (algorithm)  // converts to grayscale
                        {
                            case 0: // cie_601
                                {
                        ...
                        for(int w = 0; w < bitmap_width; w++)  // index_size = number of pixels
                        {
                            ...
                            {
                                if (colour_palette[i] == (byte)(Colours[w] >> 8) && colour_palette[i + 1] == (byte)Colours[w])
                                {
                                    diff_min_index = (byte)(i / 2);
                                    break;
                                }
                                ...
                                {
                                    diff = (short)(Math.Abs(colour_palette[i] - (byte)(Colours[w] >> 8)) + Math.Abs(colour_palette[i + 1] - (byte)Colours[w]));
                                    if (diff < diff_min)
                                    {
                                        diff_min = diff;
                                        diff_min_index = (byte)(i / 2);
        ...
        public byte[] create_PLT0_RGB565(byte[] bmp_image, int bmp_filesize, int pixel_data_start_offset)
        {
            short pixel;
            List<short> Colours = new List<short>();
            List<int[]> Colour_Table = new List<int[]>();
            int[] colour = { 0, 0 };
            for (int i = 0; i < 65536; i++)
            {
                colour[1] = i;
                Colour_Table.Add(colour.ToArray());
            }
            byte[] index = new byte[index_size];
            short diff_min = 500;
            short diff = 0;
                ...
                case 32:
                    {
                        ...
                        Colour_Table.Sort(new IntArrayComparer());
                        short j = 0;
                        Console.WriteLine("findind most used Colours");
                        for (int i = 0; i < colour_number; i++)
                        {
                            colour_palette[j] = (byte)(Colour_Table[i][1] >> 8);
                            colour_palette[j + 1] = (byte)(Colour_Table[i][1]);
                            j += 2;
                        }
                        j = 0;
                        Console.WriteLine("creating indexes");
