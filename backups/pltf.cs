...
namespace plt0
{
    ...
    class plt0_class
    {
        ...
        public void parse_args()
        {
            ...
            for (int i = 1; i < args.Length; i++)
            {
                ...
                switch (args[i].ToUpper())
                {
                    ...
                    case "--MIN":
                        {
                            if (args[i + 1].ToUpper() == "NN" || args[i + 1] == "0" || args[i + 1].ToUpper().Substring(0, 7) == "NEAREST")  // no need to check for linear as it's already the default value and will fall in its own case 
                            {
        ...
        public void write_TEX0(List<List<byte[]>> index_list)  // index_list contains all mipmaps.
        {
            ...
            data[28] = (byte)(settings[0][2] >> 8);  // unsigned short width
            data[29] = (byte)settings[0][2]; // second byte of width
            data[30] = (byte)(settings[0][3] >> 8);  // height
            data[31] = (byte)settings[0][3];
