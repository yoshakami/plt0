            ...
            if (bmd_file)
            {
                ...
                using (System.IO.FileStream file = System.IO.File.Open(input_file2, System.IO.FileMode.Open, System.IO.FileAccess.Read))
                {
                    ...
                    for (z=0; z < string_count_5 ; z++)
                    {
                        ...
                        int data_offset = (j3d[y + 0x20 + (0x20 * z) + 0x1C] << 24) + (j3d[y + 0x20 + (0x20 * z) + 0x1D] << 16) + (j3d[y + 0x20 + (0x20 * z) + 0x1E] << 8) + (j3d[y + 0x20 + (0x20 * z) + 0x1F]);
                        data_offsets.Add(data_offset);
                    ...
                    using (System.IO.FileStream ofile = System.IO.File.Open(output_file + ".bmd", mode, System.IO.FileAccess.Write))
                    {
                        ...
                        if (f != 0)
                        {
                            ofile.Write(j3d, y, f * 0x20);
                        }
                        ofile.Write(data, 0, 32);
                        if (f != string_count - 1)
                        {
                            ofile.Write(j3d, x, (string_count - f - 1) * 0x20);
                        }
                        ofile.Close();
